...
``IFile``s with a `.java` extension, or ``IProject``s with a Java nature. It will
...
In Eclipse, the use of a menu:Navigate[Show In] command is a common way
...
A menu:Navigate[Show In Navigator] command should be included in any view
...
Plug-ins should support kbd:[F1] keyboard command and link it to an infopop
...
`{ResourceListSelectionDialog}[ResourceListSelectionDialog]`.
...
`LabelProvider` for the `TableViewer`. It also has btn:[Select{nbsp}All] and btn:[Deselect{nbsp}All]
buttons.
...
Have you used the Open Type (kbd:[Ctrl + Shift + T]) or Open Resource
(kbd:[Ctrl + Shift + R]) dialog? It's similar to the `ElementListSelectionDialog`,
...
abstract class
`{FilteredItemsSelectionDialog}[FilteredItemsSelectionDialog]`
