...
*_Create_* or *_New_* is represented by a sparkle in the upper-right
  ...
  top and flush with the right edge of the 16&times;16 icon space. Using
...
Objects are stacked vertically, often in large number, within
  ...
  object images, should be located in the same place within the 16&times;16
...
link:media/common_icon_elements.zip[Download] the
...
link:media/common_wizard_elements.zip[Download] the
...
. Use the marquee tool to select all the enabled versions of the
...
. Start the "Create Disabled State" action by clicking on the "play"
...
in each image, especially for small 16&times;16 icons. This will help ensure
...
interface graphics are 16&times;16 pixels in size, though there are some
...
These overlays are displayed in the Navigator and the Package views.
  ...
  corner of the 16&times;16 icon space.
+
...
These overlays are displayed in the Outline, Hierarchy, and Package
  ...
  they extend the 16&times;16 icon space. They are placed to the right of a
  model object icon, overlapping the 16&times;16 model object space by 3
...
These overlays are displayed in the Navigator view and in the
  ...
  model object at the right of the 16&times;16 icon space.
+
When the version control overlays are displayed in the Structure View
  ...
  they extend the 16&times;16 space. They are placed to the right of a model
  object icon, overlapping the 16&times;16 model object space by 3 pixels.
...
Type:: Object Overlay
Folder name:: `ovr16`
Size:: 16&times;6 pixels image size but a final cut size of 16&times;16
  pixels
Format:: PNG
...
Type:: Table
Folder name:: `obj16`
Size:: They are designed in the 16&times;16 pixel space, but the actual
  image size is no greater than 15&times;14 pixels.
Format:: PNG
...
Size:: Size varies depending on the context of the palette. The
  default size is 16&times;16 pixels but can be changed by the user to "Use
  large icons", which are 24&times;24 pixels in size. There are also rare
  cases where 32&times;32 pixel icons are used on the palette.
Format:: PNG
...
Type:: Diagram
Folder name:: `dgm`
Size:: Canvas icons may be 16&times;16, 24&times;24, or 32&times;32 pixel in
  size. There are also instances of 15&times;15, 12&times;12, and 10&times;10 pixel
  icons in some Web Tooling diagrams. 15&times;15 and 10&times;10 icons are used
  in site navigation diagrams, and 12&times;12 icons are used in the editor
...
That is the final cut size of the image. Within that area, a
15&times;15 pixel space is reserved for the image itself, leaving both a
...
smaller than 16&times;16 pixels, it is a rule of thumb to center the image
within the 16&times;16 space. For example, a 14&times;14 pixel image will have a
single row of empty pixels on all four sides.

Exceptions to the common 16&times;16 image size are also detailed below. All
sizes are indicated with width before height.

===== Product

Product icons occupy the full space allotted for all five sizes: 16&times;16,
24&times;24, 32&times;32, 64&times;64, and 72&times;72 pixels. This shows how the
  16&times;16 product icon fills the entire space:
...
The maximum image size is 16&times;16 pixels, but 15&times;15 is recommended.
  If the image is 15&times;15 or smaller, the empty pixels must be on the
...
===== Object Overlay (and Underlay)
Most object overlay icons are a maximum image size of 7&times;8 pixels,
  ...
  15&times;16 pixels, though commonly they are a square 15&times;15 pixels in
...
*Multiplicity* object overlay with a maximum image size of 16&times;6
  pixels:
...
*Underlay* with a maximum image size of 16&times;15 pixels, but 15&times;15 is
...
The maximum image size is 15&times;14 pixels. Table icons must be no
...
===== Palette
*Standard small (16&times;16) palette icon:* The maximum image size is 16&times;15 pixels, but 15&times;15 is recommended. Palette icons must be no
...
*Standard large (24&times;24) palette icon:* The maximum image size is 24&times;23 pixels, but 23&times;23 is recommended. Palette icons must be no
...
*Large (32&times;32) palette icon:* The maximum image size is 30&times;30
...
===== Diagram
*Small (10&times;10) canvas icon:* The maximum image size is 10&times;10
...
*Small (12&times;12) canvas icon:* The maximum image size is 12&times;12
  pixels. The image fills the space as required.
...
*Small (16&times;16) canvas icon:* The maximum image size is 16&times;15
  pixels, but 15&times;15 is recommended. The empty pixels must be on the
  left and bottom, as shown here.
...
*Large (24&times;24) canvas icon:* The maximum image size is 24&times;23
  pixels, but 23&times;23 is recommended. The empty pixels must be on the
  left and bottom, as shown here.
...
*Large (32&times;32) canvas icon:* The maximum image size is 32&times;32
  pixels, but 30&times;30 is recommended with the image centered. The empty
...
The final size of the pointer and cursor masks is 32&times;32 pixels. The
  actual image size of the pointer is usually fewer than 20&times;20 pixels,
...
All wizard banner graphics are designed to fit within a specified
  screen space of 75&times;66 pixels on the right side of the wizard banner.

The actual size of each graphic will vary depending on the elements
  involved, but will generally be around 67&times;50 pixels in size.
...
To use these actions, download the
link:media/eclipse_cutting_R3V6.zip[eclipse_cutting_R3V6.atn] file, and then
...
2.  Play the __Dupe and Flatten_main file__ action to create a new, flat
file. (See _A_ in the Detailed View of Cutting Actions below)
3.  Using the _Marquee_ tool at a fixed size of 16&times;16 pixels, select the
...
menu:Save[] to save it as a `.png` in the folder you specify. (See _G_)
...
Follow the steps as laid out above, except cut the icon at 7&times;8 pixels,
using the Eclipse icon cuts_overlays action.

===== Cutting Wizard graphics

1.  Ensure that the wizard psd has a path called __wizard cut path__ under
*Paths* tab.
2.  Play the __Dupe and Flatten_main file__ action to create a new, flat
file.
...
6.  Play the __Wizard Dupe and crop__ action to create a new file that is
cropped to 75&times;66 pixels. (See __A__-__C__ below)
...
Save to save it as a `.png` in the folder you specify. (See _E_)
...
*_pal represents the default 16&times;16 pixel size and the *_pal24 and
...
contain multiple sizes of images, such as 16&times;16 and 24&times;24 pixel
