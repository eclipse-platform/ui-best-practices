...
* Submit an https://github.com/eclipse-platform/ui-best-practices/issues[issue]
* Submit a https://github.com/eclipse-platform/ui-best-practices/pulls[pull request] 
* Start a https://github.com/eclipse-platform/ui-best-practices/discussions[discussion]
...
The Workbench's main menu bar usually contains the menu:File[], menu:Edit[], menu:Navigate[],
menu:Project[], menu:Window[], menu:Help[] top-level menus. Other top-level menus that are in
between the menu:Edit[] and menu:Project[] menu are typically context specific, based
on the current active perspective, front most editor (whether active or
not), and active view..

In the menu:File[] menu you will find a menu:New[] submenu, which contains menu items
for Project, Folder, and File creation. The menu:File[] menu also contains menu
items for menu:Import[] and menu:Export[], which are used to import files into the
Workbench, and export them out again. In the menu:Edit[] menu, you will find
familiar commands like menu:Cut[], menu:Copy[], menu:Paste[], and menu:Delete[]. These commands are
...
implementation is performed by the Navigator. In the menu:Project[] menu, you
will find project related commands such as menu:Open project[], menu:Close project[]
and menu:Rebuild project[] are available. In the menu:Run[] menu, you will find
commands related to running and debugging application code, and
launching external tools such Ant scripts. In the menu:Window[] menu, you will
find the menu:Open Perspective[] submenu to open different perspectives to suit
to needs of your development tasks. You will find perspective layout
management menu items. You will also find the menu:Show View[] submenu to add
views to the current Workbench window. In addition, you will find the
menu:Preferences[] menu item, which is used to modify the functional
...
way for the object. For example, a generic menu:New[] menu item might open up
