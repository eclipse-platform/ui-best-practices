...
instance, in the following snapshot the behavior of the Show Type Hierarchy button is shown using a tool tip text.
...
enabled. In the status bar area, _Read-only_ should be shown instead of
the default _Writable_ message.
...
lifecycle, with a menu:Save[] command. This can cause confusion. The menu:File[] menu
within a workbench window contains a menu:Save[] command, but it applies to
...
menu:Save[] command within the view.
